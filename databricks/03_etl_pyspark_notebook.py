...
# MAGIC %md
# MAGIC # E-Commerce Behavioural ETL - Prepared Azure Databricks Component
# MAGIC This notebook mirrors the validated local Pandas transformation logic. Its presence in the practical package is evidence that the Databricks component was coded/prepared, not by itself evidence that an end-to-end ADF -> Databricks Azure run was successfully executed.
...
dbutils.widgets.text("sampleSize", "100000")
dbutils.widgets.text("storageAccount", "REPLACE_WITH_STORAGE_ACCOUNT")
dbutils.widgets.text("fileSystem", "ecommerce")
dbutils.widgets.text("inputFile", "ecommerce_100000.csv")

SAMPLE_SIZE = int(dbutils.widgets.get("sampleSize"))
STORAGE_ACCOUNT = dbutils.widgets.get("storageAccount")
FILE_SYSTEM = dbutils.widgets.get("fileSystem")
INPUT_FILE = dbutils.widgets.get("inputFile")
BASE = f"abfss://{FILE_SYSTEM}@{STORAGE_ACCOUNT}.dfs.core.windows.net"
INPUT = f"{BASE}/bronze/{INPUT_FILE}"
...
clean.write.mode("overwrite").parquet(SILVER)
...
).withColumn("total_interactions", F.col("views") + F.col("carts") + F.col("purchases") + F.col("remove_from_cart")) \
 .withColumn("engagement_score", F.col("views") + 3 * F.col("carts") + 5 * F.col("purchases"))
q33, q67 = engagement_base.approxQuantile("engagement_score", [0.33, 0.67], 0.001)
engagement = engagement_base.withColumn(
    "engagement_level",
    F.when(F.col("engagement_score") <= F.lit(q33), "Low")
     .when(F.col("engagement_score") <= F.lit(q67), "Medium")
     .otherwise("High")
)
engagement.write.mode("overwrite").parquet(f"{GOLD}/customer_engagement")
...
counts = stage_times.agg(F.sum("has_view").alias("view_sessions"), F.sum("reached_cart").alias("cart_sessions"), F.sum("reached_purchase").alias("purchase_sessions")).first()
...
def rate(n, d): return float(n / d * 100.0) if d else 0.0
...
funnel.write.mode("overwrite").parquet(f"{GOLD}/conversion_funnel")
...
).withColumn("abandonment_rate_pct", F.when(F.col("cart_sessions") > 0, F.col("abandoned_sessions") / F.col("cart_sessions") * 100).otherwise(0.0))
abandonment.write.mode("overwrite").parquet(f"{GOLD}/cart_abandonment")
...
).withColumn("view_to_cart_rate_pct", F.when(F.col("views") > 0, F.col("cart_additions") / F.col("views") * 100).otherwise(0.0)) \
 .withColumn("purchase_conversion_rate_pct", F.when(F.col("views") > 0, F.col("purchases") / F.col("views") * 100).otherwise(0.0))
category.write.mode("overwrite").parquet(f"{GOLD}/category_performance")

elapsed = time.perf_counter() - start
perf = spark.createDataFrame([(SAMPLE_SIZE, raw_count, clean_count, float(elapsed), float(clean_count / elapsed if elapsed else 0))],
                             ["sample_size", "raw_rows", "clean_rows", "processing_seconds", "throughput_rows_per_sec"])
perf.write.mode("append").parquet(f"{BASE}/gold/pipeline_performance")
...
result = {
    ...
    "removed_rows": int(raw_count - clean_count),
    "processing_seconds": round(float(elapsed), 4),
    "throughput_rows_per_sec": round(float(clean_count / elapsed if elapsed else 0), 2),
    "silver_path": SILVER,
    "gold_path": GOLD,
}
print("DATABRICKS RUN COMPLETED")
